/**
 * SOURCE OF TRUTH: src/utils/daily-recommendation-math.ts — server-local port (DRY debt,
 * reconcile if logic changes). Verbatim port — pure, no external deps.
 */
 ...
 * - total<35 -> all-easy: warm/cool folded into the main set.
...
export function computeDurationBreakdown(
  ...
  let warmup = clamp(
    Math.round(total * WARMUP_FRACTION),
    WARMUP_MIN_CLAMP,
    WARMUP_MAX_CLAMP,
  );
  let cooldown = warmup;
